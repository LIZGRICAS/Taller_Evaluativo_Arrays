...
const products = productsDetails.map((product)=>{return product.title});

console.log(products)

// 2. Cree una funcion para filtar los productos con descuentos mayores al 12%

const discount = productsDetails.filter((discount)=>{return discount.discountPercentage > 12})

console.log(discount)

// 3. Cree una funcion que retorne la cantidad de stock existente en los productos de la categoria smartphones

const stockDetails = productsDetails.map((stockDetails)=>{return {
  id: stockDetails.id,
  title: stockDetails.title, 
  stock: stockDetails.stock,
  category: stockDetails.category,
}}).filter((stockDetails)=>{return stockDetails.category === 'smartphones'});
...
const newProducts = productsDetails.filter((newProducts)=>{return newProducts.category === 'fragrances'});

newProducts.forEach((category1) =>{ category1.category = 'parfums'});

console.log(newProducts)

//5. Cree un nuevo array con los productos ordenados por precio.

const orderPrice = productsDetails.sort((a,b)=>{
  if (a.price > b.price) {
    return 1;
}if(a.price < b.price) {
  return -1;
